...
const SingleProduct = (props) => {
  ...
  const numOfItems = useSelector((state) => state.products.numOfItems);
    ...
    if (productId === products[i].id) {
      for (let x = 0; x < 3; x++) {
        images.push(
          <SwiperSlide
            tag="li"
            key={products[i].id * Math.random()}
            style={{ listStyle: "none" }}
          >
            <img
              src={products[i].gallarey[x].url}
              alt=""
              loading="lazy"
              width="520"
            />
          ...
          <div className="product_details">
            ...
            <div className="items_count">
              <button
                onClick={() =>
                  numOfItems > 1
                    ? dispatch(decreaseNumOfItems(numOfItems))
                    : false
                }
              >
                -
              ...
              <button
                onClick={() =>
                  numOfItems >= 0
                    ? dispatch(increaseNumOfItems(numOfItems))
                    : false
                }
              >
                +
  ...
  return (
    <React.Fragment>
      <section className="single_product">{section}</section>
      {/* <RelatedProducts /> */}
    </React.Fragment>
  );
};
